...
## Implemented (2026-06, itérations 52-56)
- **Lot 2 statuts entretien (UI)** : badges « Entretien fixé/fait » + pastille CV (verte=scanné, rouge=non scanné, libellé « CV » seul) + filtres (statut entretien / scan CV) dans la liste Utilisateurs ; « + Ajouter une candidature » depuis le profil. Testé 100% (iter_31).
- **Anglais par défaut** : `i18n.fallbackLng="en"`, détection `localStorage` uniquement (toggle FR/EN conservé). Dictionnaire `uiDict.js` enrichi (Lot 2, scan CV, agent IA, filtres, formulaire d'édition). Libellé FR raccourci « Entretien déjà fait » → « Entretien fait ».
- **Audit + durcissement sécurité** : (a) code de reset retiré des logs ; (b) **anti-XSS stocké** sur `/files/{id}` + `/files/public/{id}` (Content-Disposition attachment + octet-stream sauf allowlist images/PDF + `X-Content-Type-Options: nosniff`) ; (c) allowlist type CV (PDF/DOC/DOCX) + extensions assainies ; (d) **purge RGPD** complète (`core.purge_user_data`: applications, messages, cv_data, files, interviews, contracts, notifications, sessions) sur les 3 chemins de suppression. Testé 17/17 (iter_32).
- **Édition candidature inline (profil)** : bouton « Modifier » sur chaque candidature du profil candidat → panneau éditable (statut + appréciation + note interne + note d'entretien si interview_done) via `PUT /status`, `/review`, `/interview-note`. Testé 100% (iter_32).
- **Fallback IA multi-fournisseurs (Option A, séquentiel)** : `core.gemini_generate` devient un orchestrateur transparent **Gemini → DeepSeek → Mistral → OpenAI** (bascule sur quota/429/timeout/5xx/vide ; une seule IA à la fois ; fournisseurs sans clé ignorés ; noms de fonction conservés → zéro changement routeurs). Fallback OpenAI ajouté aussi pour la **vision (scan CV)** et l'**audio (transcription)**. Clés `.env` (vides) : `DEEPSEEK_API_KEY`, `MISTRAL_API_KEY`, `OPENAI_API_KEY` (+ modèles surchargables). Modèles par défaut : deepseek-chat / mistral-large-latest / gpt-4o (vision gpt-4o, STT whisper-1). Test unitaire 5/5 (`tests/test_llm_fallback.py`).
  - ⚠️ **Quota Gemini gratuit ACTUELLEMENT ÉPUISÉ** (429, limite 20 req/jour) → en préview, les features IA échouent tant qu'aucune clé de fallback n'est ajoutée (ou reset quotidien du quota). Ajouter au MOINS une clé (DeepSeek recommandé) à `.env` au déploiement pour garantir la continuité.
- **Compteurs & dates (itération 33)** : dashboard admin `/admin/stats` renvoie désormais `interviews_scheduled` + `interviews_done` (widget « Entretiens fixés » affiche l'effectif réel). Colonne **Date** (jj/mm/aaaa HH:mm) ajoutée à la liste des candidatures ; colonne **Inscription** (jj/mm/aaaa) ajoutée à la liste des utilisateurs (colSpan snippet CV 9→10). Testé (iter_33).
- **Flux rendez-vous (itération 34)** : dans le détail d'une candidature, « **Accepter** » change juste le statut en « Accepté » (plus aucune fenêtre) ; c'est désormais « **Entretien fixé** » qui ouvre la fenêtre de fixation de rendez-vous (date/heure/lieu/notes) puis notifie le candidat.
